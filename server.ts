...
import { createChatProvider, mergedProviderSettings } from "./src/providers/registry";
...
const LOCAL_AGENT_TOKEN = process.env.MYRAA_AGENT_TOKEN || crypto.randomUUID();
...
function spawnDesktopAgent(): void {
  ...
  const agentEnv = {
    ...
    MYRAA_AGENT_TOKEN: LOCAL_AGENT_TOKEN,
    MYRAA_APP_EXECUTABLE: process.env.MYRAA_APP_EXECUTABLE || "myraa",
  };
  ...
  try {
    const child = spawn(
      ...
      { cwd: process.cwd(), detached: false, stdio: "ignore", windowsHide: true, env: agentEnv }
    );
...
async function callDesktopAgent(
  ...
  try {
    ...
    const res = await fetch(`${DESKTOP_AGENT_URL}/execute`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-MYRAA-Token": LOCAL_AGENT_TOKEN },
...
async function startServer() {
  ...
  app.get("/api/providers", (_req, res) => {
    ...
    res.json({
      profiles: settings.profiles.map(profile => ({ ...profile, secretReference: profile.secretReference ? "configured-separately" : undefined })),
  ...
  app.post("/api/providers/settings", (req, res) => {
    try {
      ...
      if (!payload || !Array.isArray(payload.profiles) || !payload.roles || typeof payload.roles !== "object") {
        return res.status(400).json({ error: "Provider settings require profiles and roles." });
      }
      const existingProfiles = mergedProviderSettings().profiles;
      const profiles = payload.profiles.map((profile: any) => {
        const existing = existingProfiles.find(item => item.id === profile.id);
        return profile.secretReference === "configured-separately" && existing?.secretReference
          ? { ...profile, secretReference: existing.secretReference }
          : profile;
      });
      setProviderSettings({ profiles, roles: payload.roles });
      res.json({ ok: true, ...mergedProviderSettings() });
    } catch (error: any) {
      res.status(500).json({ error: error?.message || "Failed to save provider settings." });
    }
  });

  app.post("/api/providers/secret", (req, res) => {
    try {
      const reference = String(req.body?.reference || "").trim();
      const secret = String(req.body?.secret || "").trim();
      if (!reference || !secret || !/^[a-zA-Z0-9_-]+$/.test(reference)) {
        return res.status(400).json({ error: "A valid secret reference and secret are required." });
      }
      setProviderSecret(reference, secret);
      res.json({ ok: true });
    } catch (error: any) {
      res.status(500).json({ error: error?.message || "Failed to save provider secret." });
    }
  });
  ...
  app.post("/api/providers/test", async (req, res) => {
    try {
      ...
      const profile = mergedProviderSettings().profiles.find(item => item.id === profileId);
      if (!profile) return res.status(404).json({ error: "Provider profile was not found." });
      const result = await createChatProvider(profile).testConnection();
      res.status(result.ok ? 200 : 502).json(result);
    } catch (error: any) {
      res.status(502).json({ ok: false, message: error?.message || "Provider connection failed." });
    }
  });

  app.post("/api/providers/validate-tools", (req, res) => {
    const profileId = String(req.body?.profileId || "");
    const profile = mergedProviderSettings().profiles.find(item => item.id === profileId);
    if (!profile) return res.status(404).json({ ok: false, error: "Provider profile was not found." });
    const warnings: string[] = [];
    if (profile.capabilities.chat !== true) warnings.push("This profile does not advertise chat support.");
    if (profile.capabilities.tools !== true) warnings.push("This model does not advertise tool/function calling; computer control is disabled.");
    if (profile.capabilities.streaming !== true) warnings.push("Streaming is unavailable for this profile.");
    res.json({ ok: warnings.every(warning => !warning.includes("tool/function")), profileId, warnings });
  });

  app.post("/api/providers/chat", async (req, res) => {
    try {
      const settings = mergedProviderSettings();
      const profileId = String(req.body?.profileId || settings.roles.assistantProfileId || "");
      const profile = settings.profiles.find(item => item.id === profileId);
      if (!profile) return res.status(404).json({ error: "Provider profile was not found." });
      const provider = createChatProvider(profile);
      const response = await provider.chat({ messages: Array.isArray(req.body?.messages) ? req.body.messages : [], tools: req.body?.tools });
      res.json({ profileId, ...response });
    } catch (error: any) {
      res.status(502).json({ error: error?.message || "Provider chat request failed." });
    }
  });
  ...
  app.get("/api/agent-health", async (_req, res) => {
    try {
      ...
      if (r.ok) {
        ...
        res.json({ online: true, tool_count: d.tool_count, capabilities: d.capabilities || {} });
      } else {
  ...
  wss.on("connection", async (clientWs) => {
    ...
    try {
      ...
      const baseInstructions = 
        ...
        "   - You have real-time control of TECH's Linux computer through your local desktop agent (a Python backend running on this machine). When the user asks you to perform an action on their computer, DO IT immediately and naturally — like a true JARVIS-class companion.\n" +
        ...
        "   - AUTO-START: Use 'enableAutoStart' when the user wants MYRAA to start with Linux, 'disableAutoStart' to remove it, 'getAutoStartStatus' to check. Explain what you're doing.\n" +
          ...
          tools: [
            {
              functionDeclarations: [
                ...
                },
                {
                  ...
                  description: "Disable MYRAA auto-start on Linux login. Removes the startup entry.",
                  parameters: { type: Type.OBJECT, properties: {} }
                },
                {
                  name: "getAutoStartStatus",
                  description: "Check whether MYRAA is currently configured to auto-start on Linux login.",
                  parameters: { type: Type.OBJECT, properties: {} }
                }
              ]
            }
          ...
          onmessage: (message: LiveServerMessage) => {
            ...
            if (message.toolCall?.functionCalls) {
              for (const fc of message.toolCall.functionCalls) {
                ...
                const routedToolName = ({
                  browserOpen: "desktopBrowserOpen",
                  browserSearch: "desktopBrowserSearch",
                ...
                } as Record<string, string>)[fc.name] || fc.name;
                ...
                } else if (DESKTOP_TOOLS.has(routedToolName)) {
                  // ── Desktop control tools: route to Python agent ──
                  (async () => {
                    console.log(`[Desktop Agent] Routing ${fc.name} to Python backend...`);
                    const agentResult = await callDesktopAgent(routedToolName, fc.args as Record<string, unknown>);
