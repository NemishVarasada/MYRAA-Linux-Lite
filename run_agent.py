"""
VAANI Desktop Control Agent — frozen entrypoint.

This is the script PyInstaller freezes into `vaani-agent.exe`. It runs the
...
Run (frozen):   vaani-agent.exe
Run (dev):      python run_agent.py
Environment:
    VAANI_AGENT_HOST   default 127.0.0.1
    VAANI_AGENT_PORT   default 8765
    VAANI_DATA_DIR     where logs/ is written (default: cwd)
"""
...
def _resolve_data_dir() -> Path:
    data = os.environ.get("VAANI_DATA_DIR") or os.getcwd()
    logs = Path(data) / "logs"
    try:
...
def main() -> None:
    ...
    log = logging.getLogger("vaani.agent.boot")

    host = os.environ.get("VAANI_AGENT_HOST", "127.0.0.1")
    port = int(os.environ.get("VAANI_AGENT_PORT", "8765"))
    frozen = getattr(sys, "frozen", False)
    log.info("Starting VAANI agent (frozen=%s) on %s:%d", frozen, host, port)
